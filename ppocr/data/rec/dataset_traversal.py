    ...
    def __init__(self, params):
        ...
        if "distort" in params:
            ...
            if not params['use_gpu']:
                logger.info("Distort operation can only support in GPU. Distort will be set to False.")
        if params['mode'] == 'train':
        ...
        def sample_iter_reader():
            if self.mode != 'train' and self.infer_img is not None:
                ...
                if sys.platform == "win32" and self.num_workers != 1:
                    print("multiprocess is not fully compatible with Windows. num_workers will be 1.")
                    self.num_workers = 1
                if self.batch_size * get_device_num() * self.num_workers > img_num:
                    raise Exception(
                        "The number of the whole data ({}) is smaller than the batch_size * devices_num * num_workers ({})".
                        format(img_num, self.batch_size * get_device_num() * self.num_workers))
