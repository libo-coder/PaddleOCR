...
def load_params(exe, prog, path, ignore_params=[]):
    ...
    Args:
        ...
        path (string): URL string or local model path.
    ...
    logger.info('Loading parameters from {}...'.format(path))       # path: E:/project/PaddleOCR_model/MobileNetV3_large_x0_5_pretrained/...
...
def init_model(config, program, exe):
    ...
    if checkpoints:
        if os.path.exists(checkpoints + '.pdparams'):
            ...
            raise ValueError("Model checkpoints {} does not exists, check if you lost the file prefix.".format(checkpoints + '.pdparams'))
    else:
