# coding=utf-8
"""
PaddlePaddle用Program的形式动态描述整个计算过程。这种描述方式，兼具网络结构修改的灵活性和模型搭建的便捷性，
在保证性能的同时极大地提高了框架对模型的表达能力。

用户定义Operator会被顺序的放入Program中，在网络搭建过程中，由于不能使用python的控制流，
Paddle通过同时提供分支和循环两类控制流op结构的支持，让用户可以通过组合描述任意复杂的模型。
...
default_config = {'Global': {'debug': False, }}             # False, True
    ...
    for key, value in config.items():
        if "." not in key:
            ...
            assert (sub_keys[0] in global_config), \
                ...
                "please check your running command".format(global_config.keys(), sub_keys[0])
...
def create_multi_devices_program(program, loss_var_name):
    ...
    compile_program = fluid.CompiledProgram(program).with_data_parallel(loss_name=loss_var_name,
...
def train_eval_det_run(config, exe, train_info_dict, eval_info_dict):
    ...
    if type(eval_batch_step) == list and len(eval_batch_step) >= 2:
        ...
        logger.info("During the training process, after the {}th iteration, an evaluation is run every {} iterations".
    ...
    train_stats = TrainingStats(log_smooth_window, train_info_dict['fetch_name_list'])
    best_eval_hmean = -1
    best_batch_id = 0
    best_epoch = 0
    ...
    for epoch in range(epoch_num):
        ...
        try:
            while True:
                t1 = time.time()
                train_outs = exe.run(program=train_info_dict['compile_program'],
                ...
                if train_batch_id > 0 and train_batch_id % print_batch_step == 0:
                    logs = train_stats.log()
                    strs = 'epoch: {}, iter: {}, {}, time: {:.3f}'.format(epoch, train_batch_id, logs, train_batch_elapse)
                    logger.info(strs)

                if train_batch_id > start_eval_step and (train_batch_id - start_eval_step) % eval_batch_step == 0:
                    ...
                    strs = 'Test iter: {}, metrics:{}, best_hmean:{:.6f}, best_epoch:{}, best_batch_id:{}'.\
                        format(train_batch_id, metrics, best_eval_hmean, best_epoch, best_batch_id)
                    logger.info(strs)
        ...
        try:
            while True:
                ...
                train_outs = exe.run(
                    ...
                    fetch_list=train_info_dict['fetch_varname_list'],
                    return_numpy=False)
                fetch_map = dict(zip(train_info_dict['fetch_name_list'], range(len(train_outs))))
                ...
                if config['Global']['loss_type'] != 'srn':
                    ...
                    acc, acc_num, img_num = cal_predicts_accuracy(config['Global']['char_ops'], preds, preds_lod, labels, labels_lod)
                else:
                    acc, acc_num, img_num = cal_predicts_accuracy_srn(config['Global']['char_ops'], preds, labels,
                ...
                train_stats.update(stats)
                if train_batch_id > start_eval_step and (train_batch_id - start_eval_step) % print_batch_step == 0:
                    logs = train_stats.log()
                    strs = 'epoch: {}, iter: {}, lr: {:.6f}, {}, time: {:.3f}'.format(epoch, train_batch_id, lr, logs, train_batch_elapse)
                    logger.info(strs)
...
def preprocess():
    ...
    place = fluid.CUDAPlace(0) if use_gpu else fluid.CPUPlace()     # 定义执行程序的设备
