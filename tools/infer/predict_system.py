...
"""
单张图片或者图像集合预测
以下代码实现了文本检测、识别串联推理，在执行预测时，需要将 image_dir 指定单张图片或者图像集合的路径
参数 det_model_dir 指定检测 inference 模型的路径
参数 rec_model_dir 指定识别 inference 模型的路径
可视化识别结果默认保存到 ./inference_results 文件夹里面

运行指令：
    # 预测 image_dir 指定的单张图像：
    ...
    # 预测 image_dir 指定的图像集合
    python tools/infer/predict_system.py --image_dir="./doc/imgs/" \
            --det_model_dir="./inference/ch_det_mv3_db/" \
            --rec_model_dir="./inference/ch_rec_mv3_crnn/"

    # 如果想用 CPU 进行预测，需设置 use_gpu 参数为 False
    python tools/infer/predict_system.py --image_dir="./doc/imgs/11.jpg" \
            --det_model_dir="./inference/ch_det_mv3_db/" \
            --rec_model_dir="./inference/ch_rec_mv3_crnn/" \
            --use_gpu=False
    ...
    # 支持空格的通用中文OCR模型
    python tools/infer/predict_system.py --image_dir="./doc/imgs_en/12.jpg" \
            --det_model_dir="./inference/ch_det_r50_vd_db/" \
            --rec_model_dir="./inference/ch_rec_r34_cd_crnn_enhance/" \
            --use_space_char=True
